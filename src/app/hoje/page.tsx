      ...
      <div className={`grid grid-cols-2 gap-3 ${canViewRevenue ? 'sm:grid-cols-5' : 'sm:grid-cols-4'}`}>
        {canViewRevenue && (
          <KpiCard
            ...
            label="Faturamento"
            value={
              loading ? '—' : salon?.revenue != null ? formatCurrency(salon.revenue) : '—'
            }
            loading={loading}
            source={avecSource}
          />
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <p className="text-sm">
              <span className="font-semibold text-warning">
                {data!.leads.whatsapp_sem_resposta} lead(s) WhatsApp hoje
              </span>{' '}
              ainda com status Novo — ver lista.
            </p>
          </div>
      ...
      <section className="flex flex-col gap-2">
        ...
        >
          <h2 className="flex items-center gap-1.5 text-sm font-medium">
            <Calendar size={15} className="text-sky-300" /> Pessoas com agenda aberta
          </h2>
...
function KpiCard({
  ...
  source?: string
  /** Esclarece o que o número conta (ex.: movimento = cabeças). */
  hint?: string
}) {
  return (
    <div

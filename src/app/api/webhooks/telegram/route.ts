...
const WELCOME_MESSAGE = `Oi! 👋 Sou a secretária virtual do ROM Club — unidade ROM Iguatemi.
...
const STAFF_ONLY_MESSAGE =
  'Este bot é exclusivo da equipe ROM Club · ROM Iguatemi. Se você é da equipe, peça ao admin para incluir seu chat ID em TELEGRAM_STAFF_CHAT_IDS.'

const SECRETARIA_PROMPT = `Você é a secretária virtual do ROM Club — unidade ROM Iguatemi — para a equipe interna.
...
Sempre mencione que os dados são da unidade ROM Iguatemi quando relevante.
...
export async function POST(req: NextRequest) {
  ...
  if (!expected) {
    if (isProduction()) return err('TELEGRAM_WEBHOOK_SECRET não configurado', 503)

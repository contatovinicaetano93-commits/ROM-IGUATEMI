...
import { formatCurrency, formatPercent, whatsAppUrl } from '@/lib/salon/format'
...
export default function RelatorioDiretoriaPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/director-report?${q}`, { cache: 'no-store' })
      const json = await res.json()
      if (!res.ok || json.error) {
        setError(json.error ?? 'Não autorizado — entre como ADMIN-IGUATEMI')
        setData(null)
        return
      }
      setData(json.data)
  ...
  async function downloadBlob(q: URLSearchParams, filename: string) {
    const res = await fetch(`/api/director-report?${q}`)
    if (!res.ok) {
  ...
  async function send0011() {
    setSending(true)
    try {
      const res = await fetch('/api/director-report', {
        method: 'POST',
    ...
    <main className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-6 px-5 py-6 lg:gap-8 lg:px-8 lg:py-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {data && data.summary.professionals === 0 && (
            <p className="mt-2 max-w-xl text-xs text-warning">
              Nenhum profissional cadastrado ainda — preencha o roster real em
              src/lib/director-report/professionals.ts para o relatório ter dados.
            </p>
          )}
        </div>
            ...
            <div className="mb-4 space-y-2 rounded-xl border border-border bg-surface/60 px-4 py-3 text-xs leading-relaxed text-muted">
              ...
              {data?.source === 'mock' && (
                <p className="text-warning">
                  Dados de demonstração (mock / síntese). Com token Avec e sync OK, a lista 0011 vem
                  ao vivo.
                </p>
              )}
            </div>
                  ...
                  <div key={pro.id} className="rounded-2xl border border-border bg-surface/50 p-4">
                    ...
                    <ul className="space-y-2 text-sm">
                      {reactivation.slice(0, 12).map((c, i) => {
                        ...
                        const wa = whatsAppUrl(phone, buildRecallWhatsAppMessage(c, pro.name))
                        return (
                          <li
                            key={`${pro.id}-${i}`}
      ...
      <p className="text-xs text-muted">
        Destino configurado em DIRECTOR_REPORT_EMAIL · cada botão envia só a etapa aberta
      </p>
    </main>
  )
}

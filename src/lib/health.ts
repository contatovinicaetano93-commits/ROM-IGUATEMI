...
export async function getHealthStatus() {
  ...
  if (connected) {
    ;[lastFast, lastFull, kpiLayers, stockLastFast, stockLastFull, hardTimeoutHits] =
      await Promise.all([
        ...
        getRecentHardPlatformTimeoutFullRuns(24).catch((e) => {
          logger.warn('health hard_timeout probe failed', {
            error: e instanceof Error ? e.message : String(e),
          })
          return [] as Awaited<ReturnType<typeof getRecentHardPlatformTimeoutFullRuns>>
        }),
      ])
  }

...
function mockBanner(report: DirectorReport) {
  ...
  return `<p style="margin:0 0 16px;padding:10px 12px;background:#fff4ce;border:1px solid #e6c200;border-radius:8px;font-size:13px;color:#5c4b00"><b>DADOS DE DEMONSTRAÇÃO (mock)</b> — não usar para decisão financeira até o mapper Avec 0011/0021 estar ativo. Fonte interna: série sintética.</p>`
}

function html0011(report: DirectorReport) {
  const unitName = getSalonUnit().name
...
async function sendOne(
  ...
  const from =
    ...
    `${getSalonUnit().name} <onboarding@resend.dev>`

...
type ApiFetchInit = RequestInit & {
   ...
   * false = forçar rede (botão Atualizar / Relatório gerência / no-store routes).
   */
  clientCache?: boolean
  /** Abort após N ms. */
  timeoutMs?: number
...
export function apiFetch(input: string, init?: ApiFetchInit): Promise<Response> {
  ...
  if (allowCache) {
    ...
    // Singleflight: reusa a promessa em voo para a mesma URL.
  ...
  const pending = fetch(input, { ...rest, credentials: 'include', signal })
    .then(async (res) => {
    ...
    .finally(() => {
      ...
      if (allowCache) inflight.delete(key)
    })

  if (allowCache) inflight.set(key, pending)
  return pending
}

...
import { statusLabelPt } from '@/lib/salon/month-status'
...
export function buildPeriodAnalyticsCsv(period: PeriodAnalytics, unit: string): string {
  const lines: string[] = [
    ...
    '',
    csvRow('Indicador', 'Valor'),
    csvRow('Receita (mês acum.)', money(period.revenue)),
    csvRow('Atendidos (mês acum.)', period.attended),
    ...
    csvRow(
      `Comparativo ${period.previous.label}`,
      money(period.previous.revenue),
